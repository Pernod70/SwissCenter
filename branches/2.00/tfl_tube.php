 ...
 **************************************************************************************************/
  ...
  foreach ($tabs as $key=>$tab)
    $tab_strip .= ($key > 0 ? ' | ' : '').'<a href="tfl_tube.php?tab='.$tab.'&hist='.PAGE_HISTORY_REPLACE.'">'.
  ...
  else
  {
    for ($i=0; $i<=count($feed[$current])-1; $i++)
    {
      ...
      echo '<tr><td bgcolor="#'.$feed[$current][$i]['BGCOLOUR'].'"><a href="tfl_tube.php?tab='.$current.'&item='.$i.'">'.font_tags(FONTSIZE_BODY).$feed[$current][$i]['NAME'].'</font></a></td>
