 ...
 *************************************************************************************************/
  ...
  function show_picker( $url="", $search, $case = '' )
  {
    ...
    echo '<td colspan=3 height="'.convert_y(6).'" align="center"><a href="'.$url.'" name="KEY_SPC">'.str('KEYBOARD_SPC').'</a></td>';
    $url = url_set_param($url,'last','KEY_DEL');
    $url = url_set_param($url,'search', rawurlencode(substr($search,0,-1)));
    echo '<td colspan=3 height="'.convert_y(6).'" align="center"><a href="'.$url.'" name="KEY_DEL">'.str('KEYBOARD_DEL').'</a></td>';
    echo '</tr><tr>';
    ...
    for ($n=1; $n<=strlen($keys); $n++)
    {
      ...
      echo '<td width="'.convert_x(4).'" height="'.convert_y(6).'" align="center"><a href="'.$url.'" name="KEY_'.$this_key.'">'.$this_key.'</a></td>';
