...
class lastfm_artist_picker extends list_picker 
{
  ...
  {
    return db_col_to_list("select distinct artist from mp3s where artist like '$search_string' order by 1 limit $start,$end");
  }
  
  function data_count( $search_string )
  {
    return db_value("select count(distinct artist) from mp3s where artist like '$search_string'");
  }
  
  function data_valid_chars( $search_string )
  {
    return strtoupper(join(db_col_to_list(" select distinct upper(substring( artist,".(strlen($search_string)).",1)) from mp3s where artist like '$search_string' order by 1")));
  }
