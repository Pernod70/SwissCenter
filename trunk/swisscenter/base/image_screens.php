 ...
 *************************************************************************************************/
  ...
  function now_playing_image_fanart( $current_track, $previous_track = '', $next_track = '', $tracks = '', $progress = false )
  {
    ...
    // Load the image and scale it to the appropriate size.
    $fanart_img_cache = cache_filename($fanart_img, convert_x(1000,SCREEN_COORDS), convert_y(1000,SCREEN_COORDS));
    if ( !file_exists($fanart_img_cache) )
      precache( $fanart_img, convert_x(1000,SCREEN_COORDS), convert_y(1000,SCREEN_COORDS) );
    ...
    $frame->resize( convert_x(850,SCREEN_COORDS), convert_y(150,SCREEN_COORDS), 0, false );
    $image->copy($frame, convert_x(100,SCREEN_COORDS), convert_y(750,SCREEN_COORDS));
    ...
    $art_x      = convert_x(50,SCREEN_COORDS);
    $art_y      = convert_y(725,SCREEN_COORDS);
    $art_w      = convert_x(200,SCREEN_COORDS);
    $art_h      = convert_y(200,SCREEN_COORDS);
    $border_col = hexdec(style_value('NOW_ART_BORDER','#FFFFFF'));
    ...
    $text_x           = convert_x(250,SCREEN_COORDS);
    $text_y           = convert_y(775,SCREEN_COORDS);
    $text_width       = convert_x(650,SCREEN_COORDS);
    ...
    if ($tracks != '')
    {
      $total_detail = convert_x(940,SCREEN_COORDS) - $image->get_text_width($tracks,$detail_text_size);
      $image->text( $tracks, $total_detail, convert_y(895,SCREEN_COORDS), $detail_text_col, $detail_text_size);
    }
