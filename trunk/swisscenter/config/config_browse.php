 ...
 *************************************************************************************************/
  ...
  // ----------------------------------------------------------------------------------
  // Displays the various browse options
  // ----------------------------------------------------------------------------------
  ...
  function browse_display($message = '')
  {
    ...
    $url = '/config/index.php?section=BROWSE&action=DISPLAY&tab=';
    ...
    // Individual settings
    form_start('index.php', 200, 'conn');
    form_hidden('section', 'BROWSE');
    form_hidden('action', 'SET_OPTS');
    form_hidden('tab', $tab);

    form_tabs(array('VIDEO', 'MUSIC', 'PHOTOS'), $url, $tab);
    ...
    switch ($tab)
    {
      ...
      case 'MUSIC':
        ...
        form_radio_static('music_composer',str('BROWSE_COMPOSER'),$option_vals,get_sys_pref('browse_music_composer_enabled','YES'),false,true);
    ...
    switch ($tab)
    {
      ...
      case 'MUSIC':
             ...
             $_REQUEST["music_composer"] == 'YES' )
        {
          ...
          set_sys_pref('browse_music_composer_enabled',$_REQUEST["music_composer"]);
